...
/*
* append css/script to head
* */
var scriptList = []; //css/script list
function appendToPage (ext, content,callback) {
...
Injector.prototype.inject = function inject (o,index) {
    var this$1 = this;
	if (o.noCache) {
...
Bowl$1.prototype.inject = function inject () {
  ...
  var batchFetch = function (group) {
    var fetches = [];
	  group.forEach(function (item,index) {
		  fetches.push(this$1.injector.inject(this$1.ingredients.find(function (ingredient) {
			  return ingredient.key === item;
		  }),index));
    });
  ...
  resolvedIngredients.forEach(function (group) {
    ...
    ret = ret.then(function() {
      return batchFetch(group).then(function () {
	      ...
	      while (scriptList.length) {
		      ...
		      if (typeof temp !== 'undefined' && index === temp.index) {
			      index++;
			      appendToPage(temp.ext, temp.content);
		      } else {
